...
} else $product_category = $_SESSION["product_category"];

$data = $service->viewProductList($product_category);
...
<html>
  <head>
    ...
    <title>Product Menu | Customer - Turbo Runner Delivery Service</title>
  ...
  <?php foreach ($data as $row) { ?>
    <div class="clearfix card">
      <a href="../../ApplicationLayer/manageOrderView/viewProductDetails.php?service_ID=<?=$row['service_ID']?>">
        <img class="card-img-top" src="<?=$row['product_image']?>" alt="" height="230" width="230">
      </a>
      <div class="card-body">
        <h4 class="card-title"><?=$row['product_name']?></h4>
        <p class="card-text"><?=$row['product_unit_price']?></p>
      </div>
    </div>
  <?php } ?>
  </body>
