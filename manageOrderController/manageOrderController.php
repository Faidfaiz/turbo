...
class manageOrderController{
    ...
    function searchProduct($search) {
	 ...
	 //This function is to retrive the product in the cart that have the same name as the search word from the service table
    function searchProductCart($search) {
        $service = new manageOrderModel();
		
        $service->search = $search;
        if ($service->searchProductCart()) {
            $message ="Result(s) found!";
        }
        else {
            $message ="No result!";
        ...
        echo "<script>alert('$message')</script>";
        return $service->searchProductCart();
    }
